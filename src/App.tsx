...
export default function App() {
  ...
  const handleGenerate = async (params: any) => {
    ...
    if (!isPaid) {
      setError('Subscription Required: Upgrading accounts unlocks generation channels.');
      return;
    }
        ...
        {user ? (
          <div className="flex items-center gap-4 bg-zinc-900/40 p-2 rounded-2xl border border-white/5 backdrop-blur-md">
            <div className="px-3 py-1">
              <p className="text-xs font-medium text-white max-w-[160px] truncate">{user.email}</p>
            ...
            <button 
              ...
              className="px-3 py-2 text-xs text-zinc-400 hover:text-white bg-zinc-800/45 hover:bg-zinc-800 rounded-xl transition duration-200 border border-white/5 flex items-center gap-1.5"
            >
              <LogOut className="w-3.5 h-3.5" /> Out
            </button>
          </div>
        ) : (
          <span className="text-xs text-zinc-500 font-mono">Authentication Required</span>
        )}
      </header>

      <main className="relative z-20 max-w-7xl mx-auto px-6 py-10">
        {!user ? (
          // 1. Sleek High-Contrast Slate Login Interface
          <div className="max-w-md mx-auto py-12">
            <motion.div 
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              className="glass-panel p-8 border-white/10 relative"
            >
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-orange-500 via-orange-600 to-orange-800" />
              ...
              <form onSubmit={handleEmailAuth} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-900 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-orange-500/50 transition duration-200"
                ...
                <button
                  type="submit"
                  className="w-full py-3 bg-orange-600 hover:bg-orange-500 text-white font-medium text-sm rounded-xl tracking-wide transition duration-200 flex items-center justify-center gap-2"
                >
              ...
              <div className="relative my-6 text-center">
                ...
                <span className="absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 bg-zinc-950 px-3 text-[10px] font-mono uppercase tracking-widest text-zinc-600">Or Connect</span>
              </div>

              <button
                type="button"
                onClick={handleGoogleAuth}
                className="w-full py-3 bg-zinc-900 hover:bg-zinc-800/80 text-zinc-300 hover:text-white border border-white/10 font-medium text-sm rounded-xl transition duration-200 flex items-center justify-center gap-2"
              >
          ...
          // 2. Stripe Premium Checkout Paywall Wall
          <div className="max-w-2xl mx-auto py-8">
            <motion.div
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
              className="glass-panel p-10 relative overflow-hidden text-center border-white/10"
            >
              ...
              <button
                ...
                className="px-8 py-4 bg-orange-600 hover:bg-orange-500 disabled:bg-orange-600/50 text-white font-semibold rounded-xl text-md transition duration-200 tracking-wide w-full max-w-md mx-auto shadow-lg shadow-orange-950/30 flex items-center justify-center gap-2.5"
              >
                {isCheckoutLoading ? (
                  <>
          ...
          // 3. Main Composition App with Active Premium Credentials
